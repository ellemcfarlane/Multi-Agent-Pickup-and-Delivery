...
import com.smac.mas.searchclient.Tuple;
...
public class MDD {
        ...
        for (int t = 0; t < node_mutex.length; t++) {
            ...
            for (int i = 0; i < countA.length; i++) {
                ...
                if (countA[i] == M2.node_t.get(t).size()) {
                    constraintsA.add(
                            new TimePlaceConstraint(agentA, M1.node_t.get(t).get(i).box, M1.node_t.get(t).get(i).time));
                    if (M1.node_t.get(t).get(i).box != M1.node_t.get(t).get(i).agent) { // extra-over constrained
                        constraintsA.add(
                                new TimePlaceConstraint(agentA, M1.node_t.get(t).get(i).agent,
                                        M1.node_t.get(t).get(i).time));
                    }
                }
            }
            ...
            for (int i = 0; i < countB.length; i++) {
                ...
                if (countB[i] == M1.node_t.get(t).size()) {
                    constraintsB.add(
                            new TimePlaceConstraint(agentB, M2.node_t.get(t).get(i).box, M2.node_t.get(t).get(i).time));
                    if (M2.node_t.get(t).get(i).box != M2.node_t.get(t).get(i).agent) { // extra-over constrained
                        constraintsB.add(
                                new TimePlaceConstraint(agentB, M2.node_t.get(t).get(i).agent,
                                        M2.node_t.get(t).get(i).time));
                    }
                }
            }
        }
    ...
    private static Tuple<ArrayList<Constraint>, ArrayList<Constraint>> getAfterGoalConstraints(MDD M1,
        ...
        for (int[] mutex : node_mutex[last]) {
            int i = mutex[1];
            constraintsB.add(
                    new TimePlaceConstraint(agentB, M2.node_t.get(last).get(i).box, M2.node_t.get(last).get(i).time));
            if (M2.node_t.get(last).get(i).box != M2.node_t.get(last).get(i).agent) { // extra-over constrained
                constraintsB
                        .add(new TimePlaceConstraint(agentB, M2.node_t.get(last).get(i).agent,
                                M2.node_t.get(last).get(i).time));
            }
        }
        ...
        for (int t = last + 1; t < M2.node_t.size(); t++) {
            for (PositionTuple pT : M2.node_t.get(t)) {
                ...
                if (hasNonEmptyIntersection(pT, M1.node_t.get(last).get(0))) {
                    constraintsB.add(new TimePlaceConstraint(agentB, pT.box, pT.time));
                    if (pT.box != pT.agent) { // extra-over constrained
                        constraintsB.add(new TimePlaceConstraint(agentB, pT.agent, pT.time));
                    }
                }
            }
        }
